            ...
            if len(arrival_q) == 0 or next_quantum <= arrival_q[-1].arrival_time:
                ...
                if current_process.this_burst() <= 0:
                    ...
                    if not current_process.done():
                        ...
                        print("time {}ms: Process {} completed a CPU burst; {} bursts to go {}".format( self.time, current_process.pid, current_process.remaining_bursts(), self.__printreadyqueueFCFS__(ready_q))) if self.time <= 9999 else None
